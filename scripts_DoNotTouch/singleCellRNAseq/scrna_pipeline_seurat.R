...
qc_violin_plot <- function(obj, cutoffs = NULL, state_label = "") {
  ...
  # Seurat's violin scale may clip a reference line below the observed data.
  # For the PBMC tutorial, explicitly train the gene axis from zero and draw
  # labeled, higher-contrast tutorial thresholds above the violin layer.
  if (identical(params$qc_preset, "pbmc3k")) {
    tutorial_gene_lines <- c(params$min_features, params$max_features)
    tutorial_gene_lines <- tutorial_gene_lines[is.finite(tutorial_gene_lines) & tutorial_gene_lines > 0]
    plots[[1]] <- plots[[1]] +
      ggplot2::scale_y_continuous(limits = c(0, NA_real_), expand = ggplot2::expansion(mult = c(0, 0.05))) +
      ggplot2::geom_hline(yintercept = tutorial_gene_lines, color = "#C62828", linetype = "dashed", linewidth = 0.95) +
      ggplot2::annotate("text", x = Inf, y = params$min_features, label = paste0("minimum = ", params$min_features), hjust = 1.08, vjust = -0.55, color = "#A41414", size = 3.1, fontface = "bold") +
      ggplot2::annotate("text", x = Inf, y = params$max_features, label = paste0("maximum = ", params$max_features), hjust = 1.08, vjust = -0.55, color = "#A41414", size = 3.1, fontface = "bold")
  }
  plots[[3]] <- plots[[3]] +
...
qc_scatter_plot <- function(obj, cutoffs = NULL, state_label = "") {
  ...
  if (identical(params$qc_preset, "pbmc3k")) {
    counts_features <- counts_features +
      ggplot2::scale_y_continuous(limits = c(0, NA_real_), expand = ggplot2::expansion(mult = c(0, 0.05))) +
      ggplot2::geom_hline(yintercept = c(params$min_features, params$max_features), color = "#C62828", linetype = "dashed", linewidth = 0.95)
  }
  if (!is.null(cutoffs)) {
...
save_marker_annotation_panels <- function(obj, marker_list, annotation_name) {
  ...
  for (panel_index in seq_along(panels)) {
    ...
    for (group_key in names(groupings)) {
      ...
      fractions <- do.call(cbind, lapply(group_levels, function(group) Matrix::rowMeans(expression[genes, grouping == group, drop = FALSE] > 0)))
      ...
      # size is the observed proportion of positive cells.
      dot_data <- data.frame(
        marker = rep(row_labels, times = NCOL(scaled)),
        group = rep(colnames(scaled), each = NROW(scaled)),
        scaled_mean = as.vector(scaled),
        percent_expressing = 100 * as.vector(fractions),
        stringsAsFactors = FALSE
      )
      dot_data$marker <- factor(dot_data$marker, levels = rev(row_labels))
      ...
      dot <- ggplot2::ggplot(dot_data, ggplot2::aes(x = .data$group, y = .data$marker)) +
        ggplot2::geom_hline(yintercept = length(row_labels) - group_breaks + 0.5, color = "#9CA3AF", linewidth = 0.35) +
      ...
      if (requireNamespace("pheatmap", quietly = TRUE)) {
        ...
        pheatmap::pheatmap(scaled, color = grDevices::colorRampPalette(c("#2166AC", "#F7F7F7", "#C51B29"))(101), breaks = seq(-2.5, 2.5, length.out = 102), cluster_rows = FALSE, cluster_cols = FALSE, annotation_row = annotation_row, annotation_colors = list(`Marker set` = marker_colors), gaps_row = group_breaks, border_color = NA, fontsize = 11, fontsize_row = 10, fontsize_col = 10, main = paste0("Marker expression by ", group_label, " (row-scaled normalized mean)"))
        grDevices::dev.off()
      } else {
