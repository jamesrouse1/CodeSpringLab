...
# RSeQC uses foss-2021b/FlexiBLAS, whereas the available RSEM build uses
# foss-2019b. Loading both stacks together leaves RSeQC linked to a FlexiBLAS
# library that is no longer on the runtime path. Start a clean module phase
# after strand detection so RSEM and all of its dependencies are consistent.
module purge
module load EBModules
