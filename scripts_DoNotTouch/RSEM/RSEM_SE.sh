...
if [[ -z "$genome_bam" || -z "$rsem_index" || -z "$out_prefix" || -z "$strand_bed" || -z "$transcript_bam" ]]; then
  echo "ERROR: usage: RSEM_SE.sh <genome BAM> <RSEM index prefix> <feature> <output prefix> <strand BED> <transcriptome BAM>" >&2
  exit 2
fi
...
# Keep the foss-2021b RSeQC/FlexiBLAS stack separate from the foss-2019b
# RSEM stack. Mixing them makes infer_experiment.py look for libflexiblas.so.3
# after the matching library path has been replaced by the older toolchain.
module purge
module load EBModules
...
ulimit -n 10000
rsem-calculate-expression -p "${SLURM_CPUS_PER_TASK:-8}" --strandedness "$strand" \
