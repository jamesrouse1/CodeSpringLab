...
if [[ -z "$runner" || ! -s "$runner" ]]; then
  runner="${SLURM_SUBMIT_DIR:-$PWD}/../scripts_DoNotTouch/MACS2/macs2_PE.sh"
fi
[[ -s "$runner" ]] || { echo "ERROR: MACS2 runner not found: $runner" >&2; exit 2; }
exec bash "$runner" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$9" "${10}"
